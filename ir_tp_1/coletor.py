...
class Coletor: # 1 para N
    def __init__(self, codigo, max_depth=2) -> None:
        self.codigo = codigo # Só uma identificação para o coletor!
        self.max_depth = max_depth # Máximo nível de aprofundamento na busca por novas páginas.
        self.urls = {} # Dicionário com as urls ({'url1': True, 'url2': False, 'url3': False, 'url4': False, 'url5': False})
        self.objects_url = [] # Lista de objetos Url
    ...
    def extract_information(self) -> None:
        ...
        for url in good_urls: # good_urls = ['url2', 'url3', 'url4', 'url5']
            ...
            if response.status_code == 200:
                self.objects_url += [Url(url, BeautifulSoup(response.text, 'html.parser'))] # Cada item dessa lista tem a url e o conteúdo das tags <title> (título da página), <h2> (conteúdos de texto), <a> (links encontrados).
