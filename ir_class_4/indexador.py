    ...
    def inverted_index_generator(self):
        for object in self.coletor.objects_url:
            for title in object.titles: # Os h2!
                # print(title.text)
                # tokens = [token.lower() for token in nltk.word_tokenize(title.text) if token.isalnum and token.lower() not in self.stop_words] # Lista de palavras relevantes!
                #tokens = [token.lower() for token.replace('\u200b', '') in nltk.word_tokenize(title.text, language='portuguese') if token.isalnum() and token.lower() not in self.stop_words] # Lista de palavras relevantes!
            ...
            for token in self.tokenized_titles:
                self.inverted_index.setdefault(token, {}).update({object.url: 1.0}) # Link e peso do token!
                # Momento de dizer a importância do token para a pesquisa
